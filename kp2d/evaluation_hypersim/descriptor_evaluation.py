...
def compute_matching_score(data, reprojection, keep_k_points=1000, center_crop=False):
    """
    ...
    # Original: true_warped_keypoints = warp_keypoints(m_warped_keypoints, np.linalg.inv(real_H))
    true_warped_keypoints, inliers_inv = warp_keypoints_frame2frame(
        m_warped_keypoints, metainfo_inv, target_frame, source_frame,
        [neg(scenter2tcenter[0]), neg(scenter2tcenter[1])], reprojection)
    ...
    # Original: true_keypoints = warp_keypoints(m_keypoints, real_H)
    true_keypoints, inliers = warp_keypoints_frame2frame(
        m_keypoints, metainfo, source_frame, target_frame, scenter2tcenter, reprojection)

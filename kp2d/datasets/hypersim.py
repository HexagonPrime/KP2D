...
class HypersimLoader(Dataset):
    ...
    """
    def __init__(self, root_dir, training_mode, data_transform=None):

        super().__init__()
        self.loading_mode = 'hdf5'
        ...
        df = pd.read_csv(os.path.join(root_dir, 'true_frames_2.csv'), names=col_list)
        self.data_transform = data_transform

        # mode can be: 'coco', 'scene' or 'cam_trajectory'
        ...
        elif training_mode == "cam_trajectory":
            df = df.groupby(["volume", "scene", "cam"])
            for group_name, df_group in df:
                # df_group = df_group.reset_index()
                items = df_group.values.tolist()
                self.files += list(combinations(items, 2))
        elif training_mode == "consecutive":
            df = df.groupby(["volume", "scene", "cam"])
            for group_name, df_group in df:
                # df_group = df_group.reset_index()
                items = df_group.values.tolist()
